...
### Longitudinal expansion and backward lab drift

A Poisson encounter process (default mean = 4 encounters over a configurable history window) generates additional encounters per patient with shared `HASTA_ID`, age-advance per offset, and sticky comorbidity flags. Per-lab history is reconstructed by a backward random walk anchored at the latest observed value, $X_{t-1} = X_t + \varepsilon_t - \mathrm{trend}\cdot\Delta t$, with $\varepsilon_t \sim \mathcal{N}(0,\sigma^2)$ and column-specific $\sigma$ taken from clinical biological-variation tables. Per-column biological coefficients of variation follow Westgard QC summaries [21] (eGFR 6%, Hb 2%, ALT/AST 15%, TG 15%; full table in `COLUMN_DRIFT`, `src/syntha/longitudinal_labs.py`). eGFR has a single secular decline of ~1%/year applied uniformly; age-stratified eGFR decline and per-KDIGO refinements are queued for a future release. Intra-encounter blood pressure is three measurements 4–6 minutes apart with a 4/2 mmHg systolic/diastolic white-coat decline and σ ≈ 3 mmHg jitter.
...
Each synthetic episode is emitted as a transaction Bundle containing Patient, Observation (including HbA1c LOINC 4548-4), Condition, Encounter, MedicationRequest, Procedure, CarePlan, DiagnosticReport (lipid LOINC 57698-3, complete blood count [CBC] 58410-2, comprehensive metabolic panel [CMP] 24323-8, iron 24350-1, BP 85354-9), RiskAssessment, and FamilyMemberHistory resources (SNOMED CT 363346000 / 237603008). The RiskAssessment carries LOINC LP184259-5 for the CCI score [19] (with LOINC 75618-7 'Estimated 10-year survival' as the prediction panel) and a calibrated 10-year mortality $P = 1 - 0.983^{\exp(0.9\cdot \mathrm{CCI})}$ per Charlson 1994 [30]; `prediction.outcome` carries SNOMED CT 419099009 (Dead) and `probabilityDecimal` (implemented in `charlson_risk_assessment`, `src/syntha/fhir/clinical_extras.py`). For a data-quality-curated cohort CCI floor effects make this estimate dominated by background mortality. PHQ-9 (LOINC 44261-6) and GAD-7 (LOINC 70274-6) are emitted as panel totals; per-item LOINCs are not currently flattened. Each Patient carries `meta.security` HL7 v3-ActReason `HTEST` plus a syntha-copula source tag (unambiguous "not a real human" marker). Turkish localization: `HumanName` from TÜİK-derived given- and family-name distributions; `Address` from TÜİK national population-weighted ISO 3166-2:TR province codes (documented as national, not Acibadem-empirical, in `CITIES_TR`); `Patient.communication.language = 'tr'`; bilingual EN/TR `Condition.code.text` (e.g., "Malign neoplazi", "Depresif bozukluk", "Anksiyete bozukluğu"); Anxiety mapped to SNOMED CT 48694002 (Anxiety (finding)).

### Privacy audit

The `syntha audit` command runs (i) a Stadler-style nearest-synthetic-neighbor MIA and (ii) a logistic-regression train-on-synthetic / test-on-real-holdout attribute-inference attack (AIA) on sensitive targets (hypertension [Hipertansiyon], diabetes [DM_Tum], hyperlipidemia [Hiperlipidemi]). The CI gate (`.github/workflows/privacy-audit.yml`) seeds `numpy.random.default_rng(42)`, splits the input 80/20 into train/holdout, fits `GaussianCopulaGenerator(random_seed=0)` on the training fold, samples a synthetic cohort of the same size as the training fold, computes Euclidean nearest-neighbor distances on StandardScaler-standardized features, and fails the build if MIA AUC exceeds `syntha.privacy.DEFAULT_MIA_THRESHOLD = 0.60` (AIA threshold 0.70). The audit runs once per CI invocation against the committed sample (`examples/sample_output/sample_episodes.csv`); no shadow-model ensemble or multi-replicate procedure is configured.

### Engineering and reproducibility

syntha is shipped as `syntha-ehr` on PyPI, `ghcr.io/ariomoniri/syntha:v0.5.8` (commit SHA pinned at the v0.5.8 git tag), and as signed cross-OS desktop installers built with Tauri 2 + Vite 8 (macOS DevID-notarized DMG, Windows code-signed installer, Linux AppImage); the minisign public key is published on the GitHub release page for v0.5.8. CI matrix: Python 3.10–3.13 on Ubuntu, with Python 3.12 cross-tested on Ubuntu, macOS, and Windows; coverage uploaded to Codecov (badge in repository README).

Figures 1–3 are regenerated by `python scripts/make_plots.py`, which samples n=5,000 synthetic rows from the bundled tolerant copula (`app/src/model_tolerant.json`, registered into `output/tolerant/models/copula_tolerant` by `bash scripts/refresh_app_model.sh`) under `SEED=42`. Tables 1–2 are reproduced by `pytest -q` and `syntha validate`. Headline Results numbers are produced by `syntha generate --input <tolerant CSV> --n 135569 --seed 42 --cohort tolerant` with a single SEED=42 realization; multi-seed variability reporting is queued for v0.6. CI privacy gate reproduced by `.github/workflows/privacy-audit.yml`; cross-OS builds by `release.yml`; container by `docker.yml`; PyPI by `pypi-publish.yml`.

## Results

### Fidelity

Per-column distributional fidelity, computed by `syntha validate` on a 100-row synthetic sample validated against the tolerant source (n_source=135,569), yielded per-column KS distances ranging from 0 (keyword_total_flags) to 0.124 (vitamin B12); maximum binary-prevalence error 0.055 (`has_rx_data`); and Spearman correlation Frobenius difference 2.94 over the 37-column analyte–comorbidity matrix. The full per-column table is in `examples/sample_output/sample_validation_report.json`. Relative-Frobenius ratio and per-cell RMSE are not currently recorded in the repository and will be added in the validation artifact for v0.6. A full-scale n=135,569 synthetic validation has not been committed to the repository (Table 2). KS at large n is used here descriptively; classical p-values are uninformative at scale.
...
**Figure 2.** Spearman correlation heatmaps for the 37-column analyte–comorbidity matrix: source cohort (left) and synthetic sample (right). Lower triangle, polyserial/tetrachoric copula recovers off-diagonal lipid–lipid, BP–BP, and liver-enzyme blocks attenuated by naive Pearson-on-codes (see ROADMAP §5.1 for the simulation rationale).
...
A Stadler-style nearest-synthetic-neighbor MIA is exercised by `.github/workflows/privacy-audit.yml` on the committed sample (`examples/sample_output/sample_episodes.csv`, 80/20 split); the CI gate fails the build at AUROC > 0.60 (`DEFAULT_MIA_THRESHOLD` in `src/syntha/privacy.py`) [10]. A full-scale strict-cohort shadow-holdout audit with replicate-level confidence intervals has not been committed to the repository. The logistic AIA on HTN, DM, and hyperlipidemia targets is run by `syntha.privacy.run_privacy_audit` and gated at AUROC > 0.70; exact AUROCs depend on the cohort used and are emitted to `out/privacy_audit.json` at CI time. This is an empirical check, not a formal differential-privacy guarantee. The pass is consistent with the small number of released sufficient statistics (≈200 quantiles per column plus one correlation matrix).

### Engineering

The v0.5.8 release ships with 92 test cases across 19 test modules; line coverage is reported via Codecov (badge in repository README). A daily install-verification workflow exercises all signed release artifacts (macOS .dmg + .sig, Windows -setup.exe + .sig, Linux .AppImage + .sig, the macOS .app.tar.gz auto-updater payload + .sig, plus latest.json and the SBOM; full list in Data and Code Availability).
...
| Metric (units) | syntha v0.5.8 | Notes |
|---|---|---|
| Source episodes — tolerant / strict (n) | 135,569 / 55,141 | Cohort fixed; copula fit on tolerant, MIA shadow holdout from strict |
| Per-column KS distance range (unitless) | 0 – 0.124 | 100-row synthetic sample vs tolerant source (`sample_validation_report.json`) |
| Max binary-prevalence error (unitless) | 0.055 | `has_rx_data` in `sample_validation_report.json` |
| Correlation Frobenius difference | 2.94 | 37-column analyte–comorbidity matrix (100-row sample) |
| MIA AUROC CI gate | ≤ 0.60 | `DEFAULT_MIA_THRESHOLD` in `src/syntha/privacy.py`; per-run AUROCs emitted to `out/privacy_audit.json` |
| Logistic AIA AUROC CI gate (HTN, DM, lipid) | ≤ 0.70 | per-run AUROCs emitted to `out/privacy_audit.json` |
| FHIR resources emitted (count) | 10 clinical + 1 Bundle wrapper | |
| Clinical modules (count) | 9 | |
| Test cases / test modules (n) | 92 / 19 | line coverage via Codecov badge in README |
| CI matrix | Python 3.10–3.13 on Ubuntu; Python 3.12 on Ubuntu, macOS, Windows | |
...
(i) Synthetic disease prevalence is substantially below Turkish epidemiologic estimates by construction of the data-quality-curated source — for example, diabetes mellitus is ~4.8% in syntha v0.5.x versus 13.7% in TURDEP-II [31], and hypertension, hyperlipidemia, and depression are similarly under-represented relative to national figures. This is expected: the pristine cohort used as the generative substrate was curated for data quality rather than to be marginally representative of the Turkish adult population. Consequently, risk-prediction models trained on syntha v0.5.x **must** be recalibrated against TÜİK and TURDEP-II marginals (and, where relevant, condition-specific national surveys) before any deployment; uncalibrated use will systematically underestimate absolute risk. (ii) syntha is not yet differentially private; a Gaussian-mechanism wrapper on the sufficient statistics is planned for v0.7. (iii) Longitudinal expansion is a Poisson + backward random walk anchored at the latest observed value, not a PADM state machine for progression; v0.8 will add state machines. (iv) The Gaussian copula reproduces the bulk of joint distributions but not the long tails; rare-event studies should be interpreted with caution. (v) ICD-10 specificity is currently at `.9` granularity for several conditions (J45.9, J44.9, N18.9, F32.9, F41.9, E78.5, E07.9, I25.9). Because syntha already models eGFR, PHQ-9, and lipid sub-fractions, refining to N18.1–N18.5 (CKD stages), F32.0–F32.2 (depression severity), and E78.0/E78.1/E78.2 is feasible and queued for v0.6. Users training auto-coder models on v0.5.x should be aware that the dataset will bias such models toward unspecified codes — a non-trivial patient-safety implication. (vi) Reference-range coverage uses two sex-stratified analytes (hemoglobin, creatinine) and 13 sex-agnostic analytes (HDL, LDL, total cholesterol, triglycerides, ALT, AST, platelets, WBC, ferritin, vitamin B12, fasting glucose, systolic BP, diastolic BP, eGFR); TSH is currently outside the reference-range checker and is queued, as is full sex stratification (HDL, ALT, ferritin). (vii) DM_Komplikasyonlu is mapped to SNOMED CT 44054006 + ICD-10 E11.8 on the assumption that complicated cases in the source are predominantly T2DM; T1DM and secondary DM are not separately modeled.
...
- **Source code**: https://github.com/ArioMoniri/syntha (Apache-2.0, tag v0.5.8; the commit SHA for v0.5.8 can be resolved with `git rev-parse v0.5.8`)
- **PyPI**: `pip install syntha-ehr==0.5.8`
- **Docker image**: `ghcr.io/ariomoniri/syntha:v0.5.8` (commit SHA pinned at the v0.5.8 git tag)
- **Signed desktop installers**: macOS `.dmg` (DevID-notarized), Windows `-setup.exe` (code-signed), Linux `.AppImage`; minisign-signed auto-updater. Verify: `minisign -V -P "$(cat pubkey.b64)" -m <installer>`; the minisign public key is published on the GitHub release page for v0.5.8.
...
10. Stadler T, Oprisanu B, Troncoso C. Synthetic data — anonymisation Groundhog Day. Proc 31st USENIX Secur Symp. 2022.
...
30. Charlson ME, Szatrowski TP, Peterson J, Gold J. Validation of a combined comorbidity index. J Clin Epidemiol. 1994;47(11):1245–1251.

31. Satman I, Omer B, Tutuncu Y, et al. Twelve-year trends in the prevalence and risk factors of diabetes and prediabetes in Turkish adults (TURDEP-II). Eur J Epidemiol. 2013;28(2):169–180. doi:10.1007/s10654-013-9771-5.
