...
- References: **31** (started at 34; removed [19] Yıldız 2022 — fabricated; removed [31] El Emam — orphaned after Privacy-audit section was honest-downgraded to drop the unverified n_shadow/30-replicates/El Emam-threshold claims; removed [32] Sengul 2016 PatenT-2 — orphaned after Limitations (i) was rewritten to drop unverifiable PatenT-2 / TEKHARF / TR-primary-care-depression comparison figures. Bibliography renumbered 1–31; all 31 entries cited inline).
- Privacy-audit section softened: dropped invented specifics (n_shadow=11,029, 30 replicates) that were not configured in `.github/workflows/privacy-audit.yml`; kept verifiable items (seed 42, 80/20 split, `GaussianCopulaGenerator(random_seed=0)`, Euclidean on StandardScaler features, `DEFAULT_MIA_THRESHOLD = 0.60`, AIA threshold 0.70).
- Limitations (i) rewritten to drop PatenT-2 30%, TEKHARF >40%, and Turkish primary-care 10–20% comparison figures (unverifiable). The qualitative claim is preserved: synthetic prevalence is below Turkish national figures by construction of the data-quality-curated source; recalibration against TÜİK and TURDEP-II [31] is required before deployment.
- Reference [10] Stadler 2022 — "(Page range to be verified at copyedit.)" parenthetical removed.
